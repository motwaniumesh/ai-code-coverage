...
# Auto-generated tests (2025-05-20T17:01:49.465466)
...
def test_triangle_area_using_heron():
    ...
    with pytest.raises(ValueError):
        Geometry.triangle_area_using_heron(0, 4, 5)
    with pytest.raises(ValueError):
        Geometry.triangle_area_using_heron(3, 4, 10)
        
def test_is_right_angled_triangle():
    assert Geometry.is_right_angled_triangle(3, 4, 5) == True
    assert Geometry.is_right_angled_triangle(5, 4, 3) == True
    assert Geometry.is_right_angled_triangle(3, 3, 3) == False
    with pytest.raises(ValueError):
