...
# Auto-generated tests (2025-05-20T18:48:02.412128)
...
def test_rectangle_area():
    ...
    assert Geometry.rectangle_area(4.5, 2.5) == 11.25
    with pytest.raises(ValueError):
        Geometry.rectangle_area(-1, 3)
    with pytest.raises(ValueError):
        Geometry.rectangle_area(2, -3)

def test_triangle_area_using_heron():
    assert Geometry.triangle_area_using_heron(3, 4, 5) == 6
    assert Geometry.triangle_area_using_heron(5, 12, 13) == 30
    with pytest.raises(ValueError):
...
def test_is_right_angled_triangle():
    ...
    assert not Geometry.is_right_angled_triangle(1, 1, 3)
    with pytest.raises(ValueError):
        Geometry.is_right_angled_triangle(-1, 2, 3)